...
Несмотря на то, что технически ничто не мешает импортировать напрямую "из кишков" фичи: ```import { event } from 'features/cart/model/'``` так делать не стоит, так как это нарушает инкапсуляцию.
...
В некоторых приложениях удобно выделять фичу "app", которая содержит глобальное состояние и логику инициализации приложения.
...
Глобальное состояние хранится внутри фич "app", "profile", "user" или подобных им фичах. Все прочее локальное состояние - внутри конкретных фич.
...
Объявленные в public.ts публичные юниты реэкспортируются во внешний мир в точке входа фичи. 
