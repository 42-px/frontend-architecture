...
Суть подхода заключается в том, чтобы вместо того, чтобы разделять приложение на "компоненты страницы" и "прочие компоненты" мы разделяем его на **фичи**.
...
view - представление
index.ts - точка входа, экспортирующее внешнее API фичи (то что фича отдает во внешний мир)
```

Каждая фича обладает конкретным публичным апи, которое определяется в ее точке входа. 
Фичи могут импортировать друг друга ТОЛЬКО через публичное апи. 
Несмотря на то, что технически ничто не мешает импортировать напрямую "из кишков" фичи: ```import { event } from 'features/cart/model/events'``` так делать не стоит, так как это нарушает инкапсуляцию.
...
В большинстве приложений удобно выделять фичу "app" (или "common"), которая содержит глобальное состояние и представления всего приложения в целом.
...
Глобальное состояние хранится внутри фичи "app". Все прочее локальное состояние - внутри конкретных фич.
...
src/index.tsx > src/init.ts > src/features/catalog/init.ts > src/fetures/catalog/model/init.ts
...
Чтобы избежать этой неприятной ситуации, мы отделяем бизнес логику в отдельный файл, который импортится непосредственно в точке входа, причем ПОСЛЕ всего остального. В этот момент все фичи уже импортированы и возможные цилкические зависимости разрешены, поэтому мы можем быть уверен что не наткнемся на undefined. 
...
В дополнение к вышесказаному, мы также рекомендуем разделять на отдельные модули объявления эвентов, сторов и эффектов, т. к. это улучшает читаемость моделей. Также бывает удобно выносить в отдельный модуль обработчики эвентов (редьюсеры), если они становятся слишком велики и ухудшают читаемость модели 
Однако, в отличие от отделения создания от инициализации, эта практика носит рекомендательный характер.

Таким образом, наша модель приобретает следующую структуру:
```
const.ts - экспортирует константы модели, которые не обязательно делать частью глобальной конфигурации
index.ts - экспортирует внешнее апи модели: сторы, эвенты и эффекты 
domain.ts - объявление домена (доменов) данной модели
state.ts - объявление сторов
events.ts - объявление эвентов
effects.ts - объявление эффектов
computed.ts - производные, вычисляемы сторы 
reducers.ts - обработчики событий
...
2. Презентационные или UI компоненты. Являются "чистыми" компонентами, реализующими определенные элементы пользовательского интерфейса. Не могут обладать доступом к состоянию приложения, локализации или роутингу. Могут содержать собственное состояние. Примеры UI-компонентов: PrimaryButton, DropDown, Modal, InputField.
...
В папке entries располагаются компоненты, которые экспортируются во внешний мир. Зачастую это страницы, но не всегда. 
...
3. Компоненты entries (pages) импортируют компоненты из containers и parts собирая их в цельный монолитный кусок интерфейса (страницу или панельку). Entries компоненты также могут обладать собственной версткой, если это удобно в конкретном случае.

В завершение, дадим еще одну рекомендацию: не бойтесь создавать большое количество контейнеров, каждый из которых присоединяет свой кусочек состояния. Зачастую это лучше, чем расположить все данные внутри одного огромного контейнера. 
...
Как видно, здесь связывается стор $accessToken с эвентами аутентификации, предоставляется имплементация для ```request``` и ```authRequest```, а также обеспечивает сохранение и чтение токена из localStorage.
...
В реальном приложении внутри эффектов-эндпоинтов будет происходить вызов ```await request()``` или ```await authRequest()```. В демо приложении же http-запросы замоканы. 
...
Этот forward автоматически установит ```$accessToken``` при успешном резолве эффекта ```signIn```, внешнему приложению не нужно об этом беспокоиться.
...
2. Импортировать эффекты для получения/мутации данных и присоединять к ним пользовательский ввод. Пример можно посмотреть в модели [sign-in](examples/react/src/features/sign-in/model/effects.ts)
...
Unit-тесты пишутся на стеке:
* mocha (test runner)
* mocha-wepack (prebuild & wepack compatibility)
* chai (assertion library)
* sinnon (mocks & spies)
* jsdom (browser api)

Так как в данной архитектуре компоненты никогда не содержат бизнес-логики и состояния, в большинстве случаев, покрытия тестами моделей более чем достаточно. 

При тестировании моделей вы просто императивно вызываете события, так как будто бы они являются пользовательским вводом, проверяете значения сторов и количество вызовов эффектов (через sinnon spies). 
