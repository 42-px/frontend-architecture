...
при изменении $products произойдет от 1 до 3 (!) реререндера (useStore обновляет состояние посредством setState используя локальное состояние компонентов). Реальное количество ререндеров зависит от батчинга обновлений реактом (реаакт может как сбатчить так и не сбатчить последовательные вызовы setState).
